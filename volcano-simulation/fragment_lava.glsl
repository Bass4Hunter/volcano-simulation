...
void main() {
    ...
    vec4 p = vec4(vs_position*3.,time*0.05);
    vec4 p1 = vec4(vs_position*2.,time*0.05);

    float noisy =  fbm(p);
    float spots = max(snoise(p1),0.);
    gl_FragColor = vec4(noisy)+vec4(5,0.5,0.,0.);
    gl_FragColor *= mix(1.,spots,0.1);
    gl_FragColor *= vec4(100,0.5,0.,0.);
    
    gl_FragColor *= texture(material.diffuseTex, vs_texcoord)
